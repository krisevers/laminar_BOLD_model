...
P_NVC = NVC_parameters(K, P_neuro)
cbf   = NVC_model(neuro[:, ::2], P_NVC)

fig = plt.figure(figsize=(4, 6))
for i in range(K):
    plt.subplot(int('41{}'.format(i+1)))
    plt.title('Layer {}'.format(i+1))
    plt.plot(cbf[:, i], lw=2, color='blue', label='E')
    plt.plot(cbf[:, i], lw=2, color='red',  label='I')
fig.text(0.5, 0.0, 'Time [ms]', ha='center')
fig.text(0.0, 0.5, 'Neuronal Response', va='center', rotation='vertical')
plt.tight_layout()
plt.show()
...
# Specify LBR model:
# --------------------------------------------------------------------------
P = LBR_parameters(K, P_NVC)  # get default parameters (see inside the function),
