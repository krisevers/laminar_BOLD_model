...
def DMF_to_CBF_model(A, P):
	'''
	function to compute the Cerebral Blood Flow (CBF) from exisiting Dynamical Neural Mean Field Models

	INPUT:
		A - Neural activity from the Dynamic Mean Field (DMF) model dim[K,T]
	...
	for t in range(int(P['T']/dt)):
	    ...
	    yn[:,0] = A[::2,t]	# excitatory
	    yn[:,1] = A[1::2,t]	# inhibitory
	    #----------------------------------------------------------------------
	    # Vasoactive signal:
